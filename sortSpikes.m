...
trials = 10; %how often the EM-algorithm runs
runs = 25; %steps of each EM-algorithm
max_K = 15; %maximum number of clusters

D = size(b,2);
K = 0; % erfordert noch Arbeit

mu_cell = cell(1,max_K);
Sigma_cell = cell(1,max_K);
loglike_cell = cell(1,max_K);
prior_cell = cell(1,max_K);
assignment_cell = cell(1,max_K);

BIC = zeros(1,max_K);
log_error = zeros(1,max_K);
k_error = zeros(1,max_K);
...
%     assignments = sum(bsxfun(@times,max_gamma,1:K),2);
    ...
    mu_cell{K} = max_mu;
    Sigma_cell{K} = max_Sigma;
    loglike_cell{K} = max_loglike;
    prior_cell{K} = max_priors;
    assignment_cell{K} = sum(bsxfun(@times,max_gamma,1:K),2);
    
    log_error(K) = -2*max_loglike;
%    k_error(K) = K*(1+D+D*(1-D)/2);
    k_error(K) = K*(1+D+D^2);
    
    %BIC(K) = -2*max_loglike + K*(1+D+D*(1-D)/2);
    BIC(K) = -2*max_loglike + K*(1+D+D^2);
    if K>1
        if BIC(K)>=BIC(K-1)
            BIC = BIC(1:K);
            break
...
best_K = find(BIC == min(BIC));
mu = mu_cell{best_K};
Sigma = Sigma_cell{best_K};
max_loglike = loglike_cell{best_K};
priors = prior_cell{best_K};
assignments = assignment_cell{best_K};

figure()
hold on
plot(BIC, 'r')
plot(log_error(1:K), 'b')
hold off
