function plot_estimators(H,H_mle,H_mm,H_cae,H_jk,BUB_est,sampleSz,s)
...
figure
ma = max(reshape([H_mle,H_mm,H_cae,H_jk,BUB_est],[],1));
mi = min(reshape([H_mle,H_mm,H_cae,H_jk,BUB_est],[],1));
...
errorbar(sampleSz,mean(BUB_est,2),std(BUB_est,[],2),'ms-','linewidth',lw,'markersize',ms), hold on
...
legend('H','MLE','MLE+MM','JK','CAE','BUB','location','southeast'); legend('boxoff')
